...
class CropRecommendationScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      // Check if we need to initialize weather
      weatherState.when(
        data: (weather) {
          if (HybridWeatherService.isWeatherDataStale(weather.lastUpdated)) {
            weatherNotifier.refreshWeather();
          }
        },
        loading: () => {},
        error: (_, __) => weatherNotifier.initializeWeather(),
      );
    });
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            color: _getStatusColor(weatherStatus['statusColor'] as String)
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                (weatherStatus['isRealApi'] as bool) ? 'Live' : 'Demo',
                                style: TextStyle(
                                  fontSize: 9,
                      ...
                      children: [
                        weatherAsync.when(
                          ...
                          error: (_, __) => GestureDetector(
                            onTap: () => _requestLocationPermission(context, ref),
                            child: Container(
                              padding: const EdgeInsets.symmetric(
                                horizontal: 8,
                                vertical: 4,
                          ...
                          child: ElevatedButton(
                            onPressed: () {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text('${crop.cropName} selected for plantation'),
                                ),
                              );
                            },
  ...
  Future<void> _requestLocationPermission(BuildContext context, WidgetRef ref) async {
    showDialog(
      context: context,
...
class _LocationPermissionDialogState extends ConsumerState<_LocationPermissionDialog> {
    ...
    return AlertDialog(
      ...
      content: const Column(
        ...
        children: [
          ...
          SizedBox(height: 12),
          Text('• Real-time temperature', style: TextStyle(fontSize: 14, color: Colors.grey)),
          Text('• Local humidity levels', style: TextStyle(fontSize: 14, color: Colors.grey)),
          Text('• Better crop suggestions', style: TextStyle(fontSize: 14, color: Colors.grey)),
        ],
      ),
      actions: [
        TextButton(
  ...
  Future<void> _handleLocationRequest() async {
    ...
    try {
      await widget.ref.read(weatherProvider.notifier).requestLocationAndUpdateWeather();
